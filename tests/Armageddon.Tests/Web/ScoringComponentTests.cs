...
public class ScoringComponentTests : TestContext
{
    ...
    public void Scoring_ShowsNoObjectivesMessage_WhenEmpty()
    {
        ...
        var cut = RenderComponent<Scoring>();
    ...
    [Fact]
    public async Task Scoring_AddObjective_CallsApiAndRefreshes()
    {
        var objectives = new List<Objective>();
        _mockClient.Setup(c => c.GetObjectivesAsync()).ReturnsAsync(() => objectives.ToList());
        _mockClient.Setup(c => c.GetRoundsAsync()).ReturnsAsync(Array.Empty<Round>());
        _mockClient.Setup(c => c.AddObjectiveAsync("Assists"))
            .Callback<string>(name => objectives.Add(new Objective { Id = 1, Name = name }))
        ...
        _mockClient.Verify(c => c.AddObjectiveAsync("Assists"), Times.Once);
    }
    ...
    public async Task Scoring_AddRound_CallsApiWithNextNumber()
    {
        ...
        var addRoundButton = cut.Find("button.btn-primary:last-of-type");
